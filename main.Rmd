...
### Introduction, setup, loading necessary libraries, and initialize variables  
```{r}
NUMBER_OF_TEST_SETS = 10
NUMBER_OF_TRAINING_ROWS = 50000
NUMBER_OF_TEST_ROWS = 5000
...
if (!require(formatdown)) install.packages("formatdown", verbose=FALSE)
...
knitr::kable(disp_df, caption = title, digits = 4)
}
...
want to keep the the run time to be somewhat reasonable, so we will build subsets that will contain `r format_numbers(NUMBER_OF_TRAINING_ROWS, digits=6, format="dcml")` and `r NUMBER_OF_TEST_ROWS` records for the training and testing datasets respectively. There will be a total of `r NUMBER_OF_TEST_SETS` testing datasets sampled from the entire training set. When training and evaluating of the various models, the same datasets will be used.
...
The accuracy average for these `r NUMBER_OF_TEST_SETS` tests is `r as.character(accum_accuracy / NUMBER_OF_TEST_SETS)` . 
...
This algorithm ws based on the k-NN (nearest neighbor). Other models should be tested for comparisons. The first alternative to attempt is random forest, which is a compute-intensive model. There are several parameters that can be tuned to adjust the model and we will need to build a multitude of trees. As a result of this, we will only use a five-fold cross validation. The results of the  `r NUMBER_OF_TEST_SETS` test cases are as follows:


```{r display rf results, echo=FALSE}

disp_fcn(rf_results, "Summary of randomforest  testing")
rf_mean <- mean(rf_results$Accuracy)

```


The average of these 10 test cases is `r accum_accuracy`. We can see that the most important feature is whether the vehicle has been damaged followed closely by whether the customer has been previous insured.
...
When we combine the two models that we have tried (knn and random forest) to determine if the combination is better than either one and run the `r NUMER_O`_TEST_SETS`test sets. However, the ensemble accuracy turns out to be `r as.character(ensemble_mean)` which is below the random forest accuracy of `r rf_mean'.
