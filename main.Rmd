...
if (!require(tidyverse)) suppressMessages(install.packages(tidyverse, verbose=FALSE))
...
# test_index <- createDataPartition(train_data$id, times=1, p = 0.001, list = FALSE)
...
x <- train_set_x[, c(-11, -6, -5, -1)]
train_knn1 <- train(x, train_set_y, method="knn", tuneGrid = data.frame(k = c(seq(3,9,2))), trControl = control)
end.time <- Sys.time()
...
train_knn2 <- train(x, train_set_y, method="knn", tuneGrid = data.frame(k = range), trControl = control)
...
train_knn <- train(x, train_set_y, method="knn", tuneGrid = data.frame(k = k_max), trControl = control)

fit_knn <- knn3(x, train_set_y, k=9)

y_hat_knn <- predict(fit_knn, x, type="class")
...
We don't touch the *test_set* until we start testing the algorithms we build.

'''

ggplot(data=df) + geom_bar(mapping = aes(x=id, y=percent, fill=id), stat="identity")

merge accuracy tables
tbls <- merge(tb1, tbl2, by=c("k", "Accuracy", "Kappa"), all=TRUE)
which.max(tbls$Accuracy)
df <- as.data.frame(train_knn1[15])



'''
df <- data.frame(c(1,2,3,4,5,6,7,8,9), c("male","male","female", "male", "female", "male","female","male","female"))
> x <- df
> colnames(x) <- c("index", "gender")
> x$gender <- ifelse(x$gender == "male",0, 1)
> x
