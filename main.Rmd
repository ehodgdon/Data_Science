...
```{r build subset, echo=FALSE, error=TRUE}
set.seed(2024)
...
knn_fcn <- function(test_set) {
  ...
  y_hat <- predict(knn_rf, test_set_x, type = "class")
...
```
The average of these 10 test cases is `r accum_accuracy`. We can see that the most important feature is whether the vehicle has been damaged followe closelyby whether thre customer has been previous insured.
```{r graph importance}
